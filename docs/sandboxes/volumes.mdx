...
`--mount-file` validates the source *kind* by following symlinks, so a symlinked source passes the CLI check and is then refused at create time unless the `follow-root-symlinks` option is set.

### File bind-mount storage

A bind mount can target a single host file as well as a directory. For a file, microsandbox shares only that file with the guest, not the files next to it, so it stages the file in a host-side **file-mount stage** first.

On Linux and macOS, a file source is resolved following no symlink in any component: each path component is opened without following a symlink, search permission is verified before a component is discarded, and `..` is preserved by checked descriptor-stack traversal rather than cancelled textually. A symlinked component is refused at create time, naming the `follow_root_symlinks(true)` SDK opt-in or the `follow-root-symlinks` CLI mount option. With the opt-in the path is canonicalized once and the **resolved** file is staged. Ancestors that are searchable but not readable (for example a `0711` directory) remain supported, with no opt-in or new error. Other Unix targets keep the legacy staging path; Windows file binds get no no-follow enforcement.

For the **local backend on Unix (Linux/macOS)** the stage is chosen per mount by filesystem identity:

- Files on the same filesystem as the sandbox directory are hard-linked into `<sandbox_dir>/file-mounts`, normally `MSB_HOME/sandboxes/<name>/file-mounts`. A hard link keeps the host inode, so writable guest changes reach the source file. This uses the **configured backend's home**, not an ambient home when you supplied an explicit backend.
- Readonly files on a different filesystem are isolated copies under `<sandbox_dir>/file-mounts` instead of the system temp directory (which may be tmpfs). These copies consume storage under that home and persist after the handle is dropped or the sandbox is stopped, until the next spawn of that name or `msb rm`. Available space there matters; microsandbox does not guarantee physical disk if you placed that home on tmpfs. A same-filesystem readonly file is a hard link, not a snapshot, so it reflects later host edits. When that hard link is **refused** — `fs.protected_hardlinks=1` on Linux refuses a link to a file you do not own and cannot both read and write — a readonly mount falls back to an isolated **copy**, so it stops reflecting later host edits.
- Writable files on a different filesystem are hard-linked beside their source, in the source's own directory, because a hard link must stay on one filesystem. That parent directory must be writable, and microsandbox errors rather than falling back to a copy. The stage directory is requested as `0700` via `mkdirat`, and microsandbox strictly validates it as private to its owner only when the held parent reports foreign control — a different owner, group/other-writable mode, or a relevant foreign ACL — rather than by filesystem type or reliability. An ordinary private local parent therefore skips the strict check, while a foreign-owned or mode-writable parent gates it; a filesystem that misreports or synthesizes ownership and permissions limits this privacy guarantee. The stage also consumes an inode in your directory until it is removed. While a source-parent stage is active it holds several open file descriptors — the source parent, the stage root's administrative and search descriptors, and the tag directory's — and these all close when the handle closes, even if an empty stage directory is retained. If a same-device hard link into the sandbox-dir stage unexpectedly returns `EXDEV` (device equality does not always imply mount-instance identity), the mount retries once in a source-parent stage. The staged entry keeps the **requested** basename byte-for-byte, and a bare relative writable cross-device source with no parent directory is still refused as before. These stages live in your own tree; an attached sandbox handle normally removes them on drop, and a detached run keeps them, but removal is not guaranteed: cleanup conservatively retains a stage when it cannot tie the recorded identity to the removal, for example when its name was replaced or when its parent lets another principal rename entries. During an explicit staging-error unwind, such a retained stage is reported: the returned error names the retained stage's last-known path and `(dev, ino)` identity rather than claiming it was removed. A cleanup failure or conservative retention on an attached-handle drop is logged rather than returned. A runtime-owned cleanup sweep is tracked in [#25](https://github.com/gregwebs/microsandbox/issues/25).

**Windows is unchanged:** file mounts use temporary staging, try a hard link there, then readonly cross-volume copies stay in that temporary root and writable cross-volume files stage beside the source. Temporary roots are removed on ordinary handle drop or kept on detach. The Unix sandbox-dir persistence and reset guarantee above does not apply, and Windows file binds get **no** no-follow enforcement.

See [CONTEXT.md](https://github.com/gregwebs/microsandbox/blob/main/CONTEXT.md) for the domain terms and [ADR 0001](https://github.com/gregwebs/microsandbox/blob/main/docs/adr/0001-runtime-owned-file-mount-stages.md) for the rationale.
