...
- Writable files on a different filesystem are hard-linked beside their source, in the source's own directory, because a hard link must stay on one filesystem. That parent directory must be writable, and microsandbox errors rather than falling back to a copy. The stage directory is created private to its owner and consumes a held file descriptor and an inode in your directory until it is removed. If a same-device hard link into the sandbox-dir stage unexpectedly returns `EXDEV` (device equality does not always imply mount-instance identity), the mount retries once in a source-parent stage. The staged entry keeps the **requested** basename byte-for-byte, and a bare relative writable cross-device source with no parent directory is still refused as before. These stages live in your own tree; they are removed when an attached sandbox handle is dropped and kept when a detached run is disarmed. During an explicit staging-error unwind, if a stage cannot be removed — for example its name was replaced, or it sits in a directory another principal could rename entries in — the returned error names the retained stage's last-known path and `(dev, ino)` identity rather than claiming it was removed. A cleanup failure on an attached-handle drop is logged rather than returned. A runtime-owned cleanup sweep is tracked in [#25](https://github.com/gregwebs/microsandbox/issues/25).
