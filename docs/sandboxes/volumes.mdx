...
- Readonly files on a different filesystem are isolated copies under `<sandbox_dir>/file-mounts` instead of the system temp directory (which may be tmpfs). These copies consume storage under that home and persist after the handle is dropped or the sandbox is stopped, until the next spawn of that name or `msb rm`. Available space there matters; microsandbox does not guarantee physical disk if you placed that home on tmpfs. A same-filesystem readonly file is a hard link, not a snapshot, so it reflects later host edits.
- Writable files on a different filesystem are hard-linked beside their source, in the source's own directory, because a hard link must stay on one filesystem. That parent directory must be writable, and microsandbox errors rather than falling back to a copy. These stages live in your own tree; they are removed when an attached sandbox handle is dropped and kept when a detached run is disarmed, and a runtime-owned cleanup sweep is tracked in [#25](https://github.com/gregwebs/microsandbox/issues/25).

**Windows is unchanged:** file mounts use temporary staging, try a hard link there, then readonly cross-volume copies stay in that temporary root and writable cross-volume files stage beside the source. Temporary roots are removed on ordinary handle drop or kept on detach. The Unix sandbox-dir persistence and reset guarantee above does not apply.
