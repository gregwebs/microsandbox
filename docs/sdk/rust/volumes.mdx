...
Bind mount a host directory into the guest. Changes in the guest are reflected on the host and vice versa. The host path must be valid UTF-8 and must not contain `,`, `:`, or `;`. A host *file* may also be bound; file binds are staged, and on the local backend on Unix the stage location and lifetime are described in [File bind-mount storage](/sandboxes/volumes#file-bind-mount-storage).
...
Prevent writes to this mount. Enforced both at the host (virtiofs server rejects writes) and in the guest (the kernel returns `EROFS`). A readonly *file* bind is not a snapshot: on the local backend on Unix a same-filesystem file is hard-linked, and only a cross-filesystem file is copied. See [File bind-mount storage](/sandboxes/volumes#file-bind-mount-storage).
